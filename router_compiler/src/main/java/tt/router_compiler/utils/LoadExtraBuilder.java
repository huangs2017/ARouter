...
public class LoadExtraBuilder {
    ...
    public LoadExtraBuilder(ParameterSpec parameterSpec) {
        ...
        builder = MethodSpec.methodBuilder("loadExtra")
    ...
    public void setElementUtils(Elements elementUtils) {
        ...
        iServiceType = elementUtils.getTypeElement("tt.router_core.template.IService").asType();
    }
...
//            TestService testService = (TestService) ARouter.getInstance().build("/main/service1").navigation();
//            testService.test();
            statement = "t." + fieldName + " = ($T) $T.getInstance().build($S).navigation()";
            builder.addStatement(statement, TypeName.get(element.asType()), Consts.ARouter, extraName);
            return;
        } else {
            //List
